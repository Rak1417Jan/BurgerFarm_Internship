...
function convertOnboardingData(inputData) {
  ...
  for (let i = 1; i < inputData.length; i++) {
    ...
    const genderRaw = formatCellValue(getValue(row, inputHeaders, "GENDER"));
    
    // Process gender and title
    let gender = "";
    let title = "";
    if (genderRaw.includes("FEMALE") || genderRaw === "F") {
      gender = "F";
      title = "Ms.";
    } else if (genderRaw.includes("MALE") || genderRaw === "M") {
      gender = "M";
      title = "Mr.";
    }
    ...
    const employeeRow = [
      nextEmployeeId, gender, title, fullName || "", "", "", 
