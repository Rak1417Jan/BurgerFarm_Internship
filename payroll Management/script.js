...
// Replace the existing downloadAllBtn event listener with this:
downloadAllBtn.addEventListener("click", () => {
  ...
  try {
    ...
    for (const [division, ws] of Object.entries(processedByDivision)) {
      ...
      XLSX.writeFile(divWb, `Processed_Attendance_${safeDivisionName}.xlsx`);
    }

    statusMessage.textContent = "Files downloaded successfully!";
    statusMessage.className = "status success";
  } catch (error) {
...
// Add this after the showDivisionDownloads function
function showFileSelection() {
  ...
  // Create checkbox for the main processed file
  if (processedData) {
    const mainFileCheckbox = createFileCheckbox("Processed_Attendance_All.xlsx", "All Employees", true);
    fileCheckboxes.appendChild(mainFileCheckbox);
  }
  
  // Create checkboxes for each division file
  for (const division of Object.keys(processedByDivision)) {
    const safeDivisionName = division.replace(/[^a-zA-Z0-9]/g, '_');
    const fileName = `Processed_Attendance_${safeDivisionName}.xlsx`;
    const checkbox = createFileCheckbox(fileName, division, false);
...
// Helper function to create a file checkbox
function createFileCheckbox(fileName, labelText, isMainFile) {
  const container = document.createElement('div');
  container.className = 'file-checkbox';
  ...
  label.textContent = isMainFile ? labelText : `${labelText} Division`;
...
// Replace the existing downloadSelectedBtn event listener with this:
document.getElementById("downloadSelectedBtn").addEventListener("click", () => {
  try {
    ...
    statusMessage.textContent = `Downloading ${checkboxes.length} file(s)...`;
    statusMessage.className = "status processing";
    
    // Download each selected file individually
    checkboxes.forEach(checkbox => {
      if (checkbox.value === "Processed_Attendance_All.xlsx" && processedData) {
        // Download main file
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, processedData, "Processed Attendance");
        XLSX.writeFile(wb, "Processed_Attendance_All.xlsx");
      } else {
        // Download division file
        const divisionName = checkbox.value.replace('Processed_Attendance_', '').replace('.xlsx', '');
        const originalDivisionName = Object.keys(processedByDivision).find(div => 
          div.replace(/[^a-zA-Z0-9]/g, '_') === divisionName
        );
        
        if (originalDivisionName && processedByDivision[originalDivisionName]) {
          const divWb = XLSX.utils.book_new();
          XLSX.utils.book_append_sheet(divWb, processedByDivision[originalDivisionName], "Processed Attendance");
          XLSX.writeFile(divWb, checkbox.value);
        }
      }
    });
    
    statusMessage.textContent = `Download initiated for ${checkboxes.length} file(s)`;
    statusMessage.className = "status success";
  } catch (error) {
