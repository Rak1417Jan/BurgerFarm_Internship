...
//     // Calculate expected Days Payable (add 1 day for OT when days > 6)
//     let expectedDaysPayable = daysPayable;
//     if (daysPayable > 6) {
//       expectedDaysPayable = daysPayable + 1;
//     }

//     // Calculate Pending Offs OT (only when days exceed month length)
//     let pendingOffsOT = 0;
//     if (expectedDaysPayable > daysInMonth) {
//       const extraDays = expectedDaysPayable - daysInMonth;
//       pendingOffsOT = extraDays * 9; // 9 hours per extra day
//       expectedDaysPayable = daysInMonth; // Cap at month length
//     }

//     // Determine if there's a discrepancy
//     const discrepancyStatus = Math.abs(total - expectedDaysPayable) < 0.01 ? "Correct" : "Wrong";
...
//       employeeInfo.DOJ || "", // Add DOJ
//       employeeInfo.Division || "", // Add Division
//       `Present:${daysPayable.toFixed(1)}`, // Days Payable
//       `Days Payable:${total.toFixed(1)}`, // Total
//       overtimeValue, // Add Total Overtime data
//       pendingOffsOT, // Add Pending Offs OT
//       discrepancyStatus // Add Discrepancy status
//     ];
...
processBtn.addEventListener("click", async () => {
  try {
    ...
    // Show division download buttons and file selection
    showDivisionDownloads();
  } catch (error) {
    console.error("Error processing files:", error);
    statusMessage.textContent = `Error: ${error.message}`;
...
// Show division download buttons and file checkboxes
function showDivisionDownloads() {
  if (Object.keys(processedByDivision).length === 0) return;
  ...
  for (const division of Object.keys(processedByDivision)) {
    ...
    // Create checkbox for this division
    ...
    label.htmlFor = `checkbox-${division}`;
    label.textContent = `Processed_Attendance_${division.replace(/[^a-zA-Z0-9]/g, '_')}.xlsx`;
    
    checkboxDiv.appendChild(checkbox);
    checkboxDiv.appendChild(label);
    fileCheckboxes.appendChild(checkboxDiv);
  }
  
  // Add checkbox for the main file
  const mainCheckboxDiv = document.createElement('div');
  mainCheckboxDiv.className = 'file-checkbox';
  ...
  mainLabel.htmlFor = 'checkbox-main';
  mainLabel.textContent = 'Processed_Attendance_All.xlsx';
  
  mainCheckboxDiv.appendChild(mainCheckbox);
  mainCheckboxDiv.appendChild(mainLabel);
  fileCheckboxes.insertBefore(mainCheckboxDiv, fileCheckboxes.firstChild);

  divisionDownloads.style.display = 'block';
  fileSelection.style.display = 'block';
}

// Download selected files button handler
// Download selected files button handler - Modified to download files separately
downloadSelectedBtn.addEventListener('click', async () => {
  try {
    if (!processedData || Object.keys(processedByDivision).length === 0) {
      statusMessage.textContent = "No processed data available. Please process files first.";
      statusMessage.className = "status error";
      return;
    }

    // Get all checked checkboxes
    const checkboxes = fileCheckboxes.querySelectorAll('input[type="checkbox"]:checked');
    
    if (checkboxes.length === 0) {
      statusMessage.textContent = "Please select at least one file to download";
      statusMessage.className = "status error";
      return;
    }

    statusMessage.textContent = `Preparing ${checkboxes.length} file(s) for download...`;
    statusMessage.className = "status processing";
    ...
    for (const checkbox of checkboxes) {
      ...
      if (value === 'main') {
        // Create and download main file
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, processedData, "Processed Attendance");
        XLSX.writeFile(wb, "Processed_Attendance_All.xlsx");
      } else {
        // Create and download division file
    ...
    statusMessage.textContent = `${checkboxes.length} file(s) downloaded successfully!`;
    statusMessage.className = "status success";
  } catch (error) {
    console.error("Error downloading selected files:", error);
    statusMessage.textContent = `Error downloading files: ${error.message}`;
    statusMessage.className = "status error";
  }
});
