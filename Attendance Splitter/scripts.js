...
//       // FORMAT FIX #1: Format all time cells to use "13:30:35" format
//       // We'll set the format for all cells in the worksheet
//       if (!worksheet['!cols']) worksheet['!cols'] = [];
      
//       // Apply time format to all potential time cells
...
//           // If this is possibly a time value (and not a header row)
//           if (R > 0 && cell.t === 'n' && cell.v < 1) {
//             // Format as time "13:30:35"
//             if (!cell.z) cell.z = 'hh:mm:ss';
//           }
//         }
//       }
      
//       // FORMAT FIX #2: Add borders to all cells
//       // This is done by modifying the cell styles
//       for (let C = range.s.c; C <= range.e.c; C++) {
//         for (let R = range.s.r; R <= range.e.r; R++) {
//           const cell_address = {c:C, r:R};
//           const cell_ref = XLSX.utils.encode_cell(cell_address);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function downloadBranchExcel(branch) {
    try {
      ...
      // Initialize workbook with appropriate settings
      newWorkbook.Workbook = {
        Views: [{ RTL: false }],
        Sheets: [{ 
          Hidden: 0,
          // Include border settings at workbook level
          PageSetup: { 
            FitToPage: true
          }
        }]
      ...
      // Get worksheet range to process cells
      const range = XLSX.utils.decode_range(worksheet['!ref']);
      
      // FORMAT FIX #1: Format all cells that could be time values and add borders to ALL cells
      for (let C = range.s.c; C <= range.e.c; C++) {
        for (let R = range.s.r; R <= range.e.r; R++) {
          const cell_address = {c:C, r:R};
          const cell_ref = XLSX.utils.encode_cell(cell_address);
          ...
          // FORMAT FIX #1: Apply time format to time cells
          if (R > 0 && cell.t === 'n' && cell.v < 1) {
            // Format as time "13:30:35"
            cell.z = 'hh:mm:ss';
          }
          ...
          // Use medium borders all around which represents "All borders" style
          cell.s.border = {
      ...
      // Set worksheet properties to enable all borders
      worksheet['!outline'] = true;
      
      // Add worksheet to workbook
      XLSX.utils.book_append_sheet(newWorkbook, worksheet, "Branch Data");
      
      // Write the Excel file with all necessary options
      const excelBuffer = XLSX.write(newWorkbook, {
        bookType: "xlsx",
        type: "array",
        cellStyles: true,
